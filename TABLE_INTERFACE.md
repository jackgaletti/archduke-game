...
Opponent hands keep a centered two-card-width footprint. Own hands keep the full four-slot grid at four or fewer cards, expanding to 130% width (125% on narrow screens) above four occupied cards. That allowance is reserved from the outset so shared piles stay still. The server preserves base holes and fills them in the existing column-major deal order. Overflow removals compact the affected row only. Transfer the longer row’s rightmost card to the shorter row’s right end only when the difference exceeds one; either row may retain the extra odd card. Column capacity is recomputed from the current occupied count, never historical storage length. Each accepted removal independently checks the pre-removal count: 5→4 may reflow into 2×2; 4→3 leaves a stable hole even within one batch. Storage slots and revisions remain stable as row/column targets change. Each row uses its own column count C: rows of at most two columns retain normal card-plus-gap spacing; longer rows use step = (handWidth − cardWidth) / (C − 1). Appending a column smoothly increases overlap without shrinking cards or moving the other zones. Successive columns stack above earlier ones. Hover/keyboard focus lifts a card; an authorized revealed face temporarily sits above the stack.
...
The waiting-room header keeps Home / ARCHDUKE / Invite in independent left, center and right regions. Home shares Invite typography and interaction styling. It waits for an authenticated server departure, clears the room cookie and admission retry cache, then navigates home. It is never rendered in active gameplay.
