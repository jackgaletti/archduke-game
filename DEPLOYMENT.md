...
4. Runtime **24.18.1**; build **`npm ci --include=dev && npm run build`**; start **`npm start`**; health path **`/healthz`**. The explicit `--include=dev` keeps build-time tools such as Sharp, TypeScript, and Vite available even though the service uses `NODE_ENV=production`. Render supplies PORT; the server binds it on `0.0.0.0`. Frontend and WebSockets share this process.
