...
export function GameTable({v,now,connected,taken,pending,loaded,error,clearError,send}:Props){
 ...
 function invalidDestination(event:InvalidMatch){
  ...
  const own=owner.id===v.you,card=own?layout.card:layout.opponentCard,gap=own?layout.gap:5;const geometry=handGeometry(card,gap,future.columns*2,own?layout.expansion:1);const step=(geometry.width-card)/(Math.max(2,future.columns)-1);
