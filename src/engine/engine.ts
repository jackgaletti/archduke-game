...
export function startRound(s:State,d:Dependencies,redeal=false){
 ...
 for(const p of s.players){p.slots=Array.from({length:4},(_,i)=>({rev:1,card:s.deck.pop()!,row:i%2,column:Math.floor(i/2)}));p.columns=2;p.initial=undefined;p.initialOpen=[];p.hidden=false;p.ready=false;p.reviewingResults=false;p.viewingLeaderboard=false;for(let i=0;i<4;i++){const start=d.now()+i*40;s.movements.push({id:ident(s),kind:'deal',from:'draw',to:endpoint(p.id,i),start,end:start+d.motion});}}
...
function prepareNextRound(s:State,d:Dependencies){
 ...
 for(const p of s.players){p.slots=[];p.reviewingResults=false;p.viewingLeaderboard=false;p.initial=undefined;p.initialOpen=[];p.hidden=false;}
...
export function tick(s:State,d:Dependencies):boolean{
 ...
 if(s.phase==='INITIAL_PEEK'&&initial){
  if(initial.stage===0&&d.now()>=initial.revealAt){const delay=d.now()-initial.revealAt;initial.revealAt+=delay;initial.hideAt+=delay;initial.finishAt+=delay;initial.stage=1;for(const p of s.players){p.initial=[1,3];p.initialOpen=[1,3];}changed=true;}
...
export function apply(s:State,d:Dependencies,actor:string,c:Command):Reply{
 ...
 if(c.type==='abort'){host();s.phase='LOBBY';s.betweenRounds=false;s.finalEndsAt=undefined;s.round=0;s.game++;s.history=[];s.deck=deck();s.discard=[];s.held=undefined;s.effects=[];s.paused=undefined;s.open=false;s.movements=[];s.reveals=[];s.winners=[];s.caller=undefined;s.lastTurn=undefined;s.finalTurns=[];s.ending=false;s.incorrectApplied=false;s.next='';s.turn=0;s.window='';s.visualUntil=d.now();s.unlockAt=d.now();s.restartAt=0;for(const q of s.players){q.slots=[];q.ready=false;q.reviewingResults=false;q.viewingLeaderboard=false;q.initial=undefined;q.initialOpen=[];q.hidden=false;}log(s,'Game returned to lobby.');}
  ...
  switch(c.type){
   ...
   case 'draw':need((s.phase==='INTER_TURN')&&s.open,'Drawing is unavailable.');need(c.window===s.window&&c.turn===s.turn,'Turn or matching window changed.','STALE');need(s.next===actor,'It is another player’s turn.');settled();need(d.now()>=s.unlockAt,'Matching opportunity is still protected.','EARLY');need(c.source==='draw'?available(s)>0:s.discard.length>0,'That pile is empty.');{const card=c.source==='draw'?supply(s,d)!:s.discard.pop()!;s.held={id:ident(s),owner:actor,source:c.source,card};s.open=false;s.phase='HOLDING_DRAWN_CARD';s.lastTurn=undefined;movement(s,d,'draw',c.source,`held:${actor}`,card.value,c.source==='draw'?actor:undefined);}break;
   ...
   case 'effect':{
    ...
    else{const targets=c.targets??[];need(targets.length===(e.kind===11?2:1),'Choose the required targets.');need(targets.every(t=>t.player!==s.caller),'The Archduke caller is immune.');const slots=targets.map(t=>slot(s,t));if(e.kind===11){need(targets[0].player!==targets[1].player,'Swap between two different players.');[slots[0].card,slots[1].card]=[slots[1].card,slots[0].card];slots.forEach(v=>v.rev++);movement(s,d,'swap',endpoint(targets[0].player,targets[0].slot),endpoint(targets[1].player,targets[1].slot));s.effects.shift();log(s,`${p.name} swapped the selected cards.`);}else if(targets[0].player===actor&&s.discard.length&&matches(slots[0].card!.value,s.discard.at(-1)!.value)){normalizeRows(p);const matched=slots[0].card!;slots[0].card=undefined;slots[0].rev++;closeRow(p,slots[0].row!);s.discard.push(matched);movement(s,d,'match',endpoint(actor,targets[0].slot),'discard',matched.value,undefined,undefined,s.discard.at(-2)?.value);log(s,`${p.name} matched a card while choosing a Peek target.`);if(!occupied(p)){endRound(s,d);break;}effect(s,actor,matched.value);}else {e.target=targets[0];e.value=slots[0].card!.value;movement(s,d,'peek',endpoint(targets[0].player,targets[0].slot),endpoint(targets[0].player,targets[0].slot));e.viewUntil=s.visualUntil+d.peek;s.unlockAt=e.viewUntil+d.delay;log(s,`${p.name} is privately inspecting a card.`);}}
    autoSkip(s);break;}
