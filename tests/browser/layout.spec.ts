...
test('2–6 players retain a fixed opponent strip, larger central cards and readable responsive bounds',async({browser,page})=>{
 ...
 for(const count of [2,3,4,5,6]){
  const link=await create(page);const pages=[page];for(let i=1;i<count;i++)pages.push(await join(browser,link,`Player ${i+1}`));if(count===6){await page.setViewportSize({width:390,height:844});await expect(page.locator('.room-lobby')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBe(390);for(const [width,height] of [[1280,800],[1440,900],[1920,1080],[820,1180],[1180,820]]){await page.setViewportSize({width,height});await expect(page.locator('.room-lobby')).toBeVisible();await expect(page.locator('.lobby-players li')).toHaveCount(6);await expect(page.locator('.card-front img')).toHaveCount(0);await page.screenshot({path:`artifacts/waiting-six-${width}x${height}.png`});}}await start(pages);await peek(pages);
