export type Config={port:number;origin:string;persistence:'memory'|'sqlite';dataDir:string;delay:number;motion:number;peek:number;grace:number;restart:number;production:boolean};
export function config(env:NodeJS.ProcessEnv=process.env):Config{
 ...
 return {port:number('PORT',3000,0,65535),origin,persistence,dataDir:env.DATA_DIR??'./data',delay:number('MATCH_DELAY_MS',1000,1000),motion:number('ANIMATION_MS',500,100,2000),peek:number('PEEK_MS',3000,1000,10000),grace:number('RECONNECT_GRACE_MS',60000,1000),restart:number('RESTART_MS',2000,1000,10000),production};
}
