...
export function createApp(config:Config,store?:Store,dependencies?:Dependencies){
 ...
 app.post('/api/create',async(req,res)=>{const input=z.object({name,id:z.string().uuid()}).strict().safeParse(req.body);if(!input.success){res.status(400).json({message:'Enter your name (1–24 characters).'});return;}
