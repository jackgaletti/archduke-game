...
export function GameTable({v,now,connected,taken,pending,loaded,error,clearError,send}:Props){
 const surface=useRef<HTMLDivElement>(null);const [resizing,setResizing]=useState(false);const [portrait,setPortrait]=useState(()=>innerHeight>innerWidth);const [selected,setSelected]=useState<Target[]>([]);const [matching,setMatching]=useState(false);const [multi,setMulti]=useState(false);const [size,setSize]=useState({width:innerWidth-64,height:innerHeight-150});const dialog=useRef<HTMLDialogElement>(null);const [inspection,setInspection]=useState<{player:string;slot:number}>();const inspector=useRef<HTMLDialogElement>(null);
 ...
 useEffect(()=>{setSelected([]);setMatching(false);setMulti(false);setInspection(undefined);},[v.phase,v.effects[0]?.id,v.game,v.round]);
 ...
 const pointsLabel=(points:number)=>`${points} ${points===1?'pt':'pts'}`;
 function cardClick(t:Target){
  ...
  if(ownEffect&&!matching){
   if(effect.kind===12){if(effect.viewUntil)send({type:'done',effect:effect.id});else send({type:'effect',effect:effect.id,targets:[t]});return;}
  ...
  if(multi)setSelected(old=>old.some(x=>x.player===t.player&&x.slot===t.slot)?old.filter(x=>x.slot!==t.slot):[...old,t]);else send({type:'match',window:v.window,slots:[t]});
 }
 function allowed(p:View['players'][number],slot:View['players'][number]['slots'][number]){
  ...
  if(ownEffect&&!matching){if(effect.kind===1)return false;if(effect.viewUntil)return effect.target?.player===p.id&&effect.target.slot===slot.index&&effect.target.rev===slot.rev;return settled&&p.id!==v.caller;}
 ...
 function renderSeat(p:View['players'][number],index:number){
  const own=p.id===v.you;const giving=ownEffect&&!matching&&effect.kind===1&&p.id!==v.you&&p.id!==v.caller;
 ...
 return <main ref={surface} className={`game ${resizing?'resizing':''} players-${v.players.length} ${layout.compact?'compact-table':''} ${v.caller&&!lobby?'final-phase':''}`} data-phase={v.phase} data-orientation={portrait?'portrait':'landscape'} data-layout-width={size.width} data-layout-height={size.height} style={{'--central-card':`${layout.central}px`,'--hand-reserve':`${layout.reserve}px`,'--primary-card':`${layout.card}px`,'--card-width':`${layout.card}px`,'--zone-gap':`${layout.zone}px`,'--outer-pad':`${layout.pad}px`,'--strip-height':`${layout.stripHeight}px`,'--header-height':`${layout.header}px`,'--opponent-width':`${Math.min(Math.max(150,layout.opponentCard*3+24),(size.width-2*layout.pad-24*(layout.stripColumns-1))/layout.stripColumns)}px`,'--strip-columns':layout.stripColumns} as CSSProperties}>
  ...
  <div className="game-feedback">{error&&<div className="notice" role="alert">{error}<button aria-label="Dismiss message" onClick={clearError}>×</button></div>}{(!connected||taken)&&<p role="status">{taken?'This seat is open in another tab.':'Reconnecting…'}</p>}{v.incompatible&&<p role="alert">{v.incompatible} <a href="/">New room</a></p>}
  {v.paused&&!v.incompatible&&<section className="pause-banner"><span>Paused · {v.paused.reason}</span>{host&&<div><button disabled={!v.players.every(p=>p.connected)||v.paused.reason==='No cards available'} onClick={()=>send({type:'resume'})}>Resume game</button>{v.paused.reason==='No cards available'&&<button onClick={()=>send({type:'redeal'})}>Redeal round</button>}<button onClick={()=>send({type:'abort'})}>End game</button></div>}</section>}{!v.paused&&now<v.restartAt&&<p role="status">Resuming in {Math.ceil((v.restartAt-now)/1000)}…</p>}</div>
