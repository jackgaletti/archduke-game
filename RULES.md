...
Any player can match their own cards while the window is open, including the last turn owner and Archduke caller. Matching is equality plus 0↔13. Hands of four or fewer retain their four base positions when matching leaves a hole. Penalty/Give cards fill empty base positions in the existing deal order (top-left, bottom-left, top-right, bottom-right) before adding overflow. Before each accepted removal, count the occupied cards. Above four, compact only the affected row, then move the longer row’s rightmost card to the shorter row’s right end only while their counts differ by more than one. Either row may retain the odd extra card. A 5→4 removal finishes as a balanced 2×2 grid; each later removal starts in stable-slot mode and leaves a hole, including removals in the same batch. Added overflow cards append to the shorter row (top on ties). Stable storage identities and occupancy revisions prevent a stale click from selecting a shifted neighbor. Replacements and Swaps retain the selected positions. Multi-card submissions process ordered distinct slots, preserving valid matches in a mixed batch.
...
Lobby Home explicitly removes the authenticated membership and its reconnect credentials, freeing its capacity slot. The host transfers to the earliest remaining participant without resetting readiness. The last departure removes the empty room. Waiting late arrivals may leave without changing the active game; active players use the existing host Exit flow instead.
