...
it('crash/restart preserves held draw, queued peek, action IDs, credentials and frozen time; memory loses room',async()=>{
 ...
 try{
  ...
  for(const p of [a,b])expect((await p.send({type:'ready',ready:true})).ok).toBe(true);if(a.v.phase==='LOBBY')expect((await a.send({type:'start'})).ok).toBe(true);
  ...
  await crash();await launch('sqlite');a=await connect(room,host.cookie);b=await connect(room,guest.cookie);await expect.poll(()=>b.v.paused).toBeUndefined();expect(b.v.players.map(p=>p.id)).toEqual(identities);expect(b.v.held?.value).toBe(heldValue);expect(a.v.held?.value).toBeUndefined();expect(await b.send(draw,drawId)).toEqual(drawReply);await delay(Math.max(0,b.v.restartAt-Date.now(),b.v.visualUntil-Date.now())+20);
  expect((await b.send({type:'resolveDraw',turn:b.v.turn,target:{player:b.v.you,slot:0,rev:1}})).ok).toBe(true);expect(b.v.effects[0].kind).toBe(12);const effect=b.v.effects[0].id;await delay(Math.max(0,b.v.visualUntil-Date.now())+20);expect((await b.send({type:'effect',effect,targets:[{player:a.v.you,slot:0,rev:1}]})).ok).toBe(true);const face=b.v.players[0].slots[0].value;expect(face).toBe(13);
  await crash();await delay(200);await launch('sqlite');a=await connect(room,host.cookie);const pausedA=a.v;b=await connect(room,guest.cookie);await expect.poll(()=>b.v.paused).toBeUndefined();expect(b.v.effects[0].id).toBe(effect);expect(b.v.effects[0].viewUntil!-b.v.restartAt).toBeCloseTo(pausedA.effects[0].viewUntil!-pausedA.paused!.since,0);expect(b.v.players[0].slots[0].value).toBe(face);expect(a.v.players[0].slots[0].value).toBeUndefined();await delay(b.v.restartAt-Date.now()+20);expect((await b.send({type:'done',effect})).ok).toBe(true);expect(b.v.effects).toHaveLength(0);
