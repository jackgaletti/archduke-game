...
test('server countdown changes only existing button text in rounds 2–4; Next shares its geometry',async({browser,page})=>{
 ...
 try{
  ...
  for(const previous of [1,2,3]){
   ...
   await seed(previous,true);for(const p of pages){await p.getByRole('button',{name:'Next',exact:true}).click();await expect(p.getByRole('dialog')).toBeVisible();await expect(p.locator('.round-label')).toHaveText(`Round ${previous} / 4`);await p.getByRole('button',{name:'Next',exact:true}).click();await expect(p.getByRole('button',{name:'Ready',exact:true})).toBeVisible();await expect(p.locator('.round-label')).toHaveText(`Round ${previous+1} / 4`);if(p===page){await expect(guest.locator('.round-label')).toHaveText(`Round ${previous} / 4`);await page.reload();await expect(page.getByRole('button',{name:'Ready',exact:true})).toBeVisible();await expect(page.locator('.round-label')).toHaveText(`Round ${previous+1} / 4`);}}
