...
for(const reduced of [false,true])for(const late of [false,true])test(`${late?'late':'incorrect'} flip, shake, close and land (${reduced?'reduced':'full'} motion) without blocking turns`,async({browser,page})=>{
 const f=await setup(page,browser,reduced);try{
  ...
  await f.advance(180);
  // Sample the fake server clock, not extra wall time spent inspecting other clients.
  await face.evaluate((el,elapsed)=>{for(const a of el.getAnimations()){if(a instanceof CSSTransition)continue;a.pause();a.currentTime=elapsed;}},f.now()-event.start);
