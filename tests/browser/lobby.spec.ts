...
test('room lobby, unchanged initial deal, late waiting entry and host Exit',async({browser,page})=>{
 ...
 try{
  ...
  for(const [width,height] of [[1280,800],[1440,900],[1920,1080],[820,1180],[1180,820],[390,844]]){
   ...
   const control=(await page.locator('.invite-control').boundingBox())!;expect(control.x).toBeGreaterThan(width/2);expect(control.y+control.height).toBeLessThanOrEqual(88);await page.screenshot({path:`artifacts/lobby-two-${width}x${height}.png`});
  }
