...
it('crash/restart preserves held draw, queued peek, action IDs, credentials and frozen time; memory loses room',async()=>{
 ...
 async function launch(mode:'memory'|'sqlite'){
  child=spawn(process.execPath,['--import','tsx','tests/restart-server.ts'],{cwd:process.cwd(),env:{...process.env,HOST_SECRET:'restart-test-secret',PUBLIC_ORIGIN:url,PORT:'3199',PERSISTENCE:mode,DATA_DIR:dir,NODE_ENV:'test'},stdio:['ignore','pipe','pipe']});
 ...
 try{
  await launch('sqlite');const host=await post('/api/create',{name:'Host',secret:'restart-test-secret'});const room=host.data.room as string;let a=await connect(room,host.cookie);const guest=await post('/api/join',{name:'Guest',invite:a.v.invite});let b=await connect(room,guest.cookie);
  for(const p of [a,b])expect((await p.send({type:'ready',ready:true})).ok).toBe(true);expect((await a.send({type:'start'})).ok).toBe(true);
  for(const p of [a,b]){await delay(Math.max(0,p.v.visualUntil-Date.now()+10));const own=p.v.players.find(x=>x.id===p.v.you)!;expect((await p.send({type:'peekInitial',slots:own.slots.slice(0,2).map(x=>({player:own.id,slot:x.index,rev:x.rev}))})).ok).toBe(true);expect((await p.send({type:'hideInitial'})).ok).toBe(true);}
