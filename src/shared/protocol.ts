...
export const commandSchema = z.discriminatedUnion('type', [
  ...
  z.strictObject({...base,type:z.literal('peekInitial'),slots:z.array(targetSchema).length(2)}),
  z.strictObject({...base,type:z.literal('hideInitial')}),
...
export type Movement = {kind:string;from:string;to:string;start:number;end:number;value?:number};
export type PublicPlayer = {id:string;name:string;connected:boolean;ready:boolean;peeked:boolean;slots:{index:number;rev:number;occupied:boolean;value?:number}[]};
...
export type View = {
 ...
 players:PublicPlayer[];next:string;turn:number;window:string;open:boolean;unlockAt:number;visualUntil:number;restartAt:number;
 drawCount:number;discardCount:number;discard?:number;held?:{owner:string;source:'draw'|'discard';value?:number};
 ...
 lastTurn?:string;acks:Record<string,Reply>;persistence:'memory'|'sqlite';
};
